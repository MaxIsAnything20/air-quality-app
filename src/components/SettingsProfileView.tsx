 ...
 * activities (see services/streak.ts) rather than inventing either.
 */
export default function SettingsProfileView({ onBack, streak, badgeEarned }: SettingsProfileViewProps) {
  return (
    <div className="flex-1 flex flex-col overflow-y-auto">
      ...
      <div className="px-4 pt-4 pb-6">
        <div className="grid grid-cols-2 gap-2.5 mb-5">
          ...
          <div className="bg-ink-100 dark:bg-night-700 rounded-2xl p-4 text-center">
            <p className="text-2xl font-semibold text-ink-900 dark:text-night-100 m-0">{badgeEarned ? 1 : 0}</p>
            <p className="text-xs text-ink-600 dark:text-night-200 m-0 mt-1">badge earned</p>
          </div>
        </div>
        ...
        <div
          className={`flex items-center gap-3 rounded-xl px-3.5 py-3 mb-6 ${
            badgeEarned ? 'bg-[#1F4D3A]/10 dark:bg-[#3C8562]/20' : 'bg-ink-100 dark:bg-night-700'
          }`}
        >
          <span
            className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${
              badgeEarned ? 'bg-[#D9922B]' : 'bg-ink-200 dark:bg-night-600'
            }`}
          >
            <svg
              width="18"
              ...
              viewBox="0 0 24 24"
              fill="none"
              stroke={badgeEarned ? 'white' : 'currentColor'}
              strokeWidth="1.8"
              className={badgeEarned ? '' : 'text-ink-400 dark:text-night-400'}
            >
              <path d="M12 15a5 5 0 1 0 0-10 5 5 0 0 0 0 10ZM8.5 13.5 7 21l5-2.5L17 21l-1.5-7.5" />
            </svg>
          </span>
          <div>
            <p className="text-sm font-medium text-ink-900 dark:text-night-100 m-0">First activity</p>
            <p className="text-[11px] text-ink-400 dark:text-night-400 m-0 mt-0.5">
              {badgeEarned ? 'Unlocked' : 'Complete your first tracked activity to unlock'}
            </p>
          </div>
        </div>
