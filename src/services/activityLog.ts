...
/**
 * Reclassifies a saved activity's type after the fact — e.g. you tagged a
 * "Walk" but it was really a "Dog walk". Matches AirTrack's ability to edit
 * a logged segment rather than only ever delete-and-redo it.
 */
export function updateActivityType(activityId: string, type: ActivityType): Activity | null {
  const activities = readAll()
  const activity = activities.find((a) => a.id === activityId)
  if (!activity) return null
...
/**
 * Trims a saved activity's GPS trace to the [startIndex, endIndex] range
 * (inclusive, 0-based) — the editable-segment equivalent of cropping out a
 * bad start (e.g. GPS drift before you actually started moving) or a bad
 * end (forgot to stop tracking). `startedAt`/`endedAt` are pulled from the
 * remaining boundary points' own timestamps so distance/duration stats
 * recompute correctly afterward. Refuses to trim below 2 remaining points
 * (an activity needs at least a start and end to have a route at all).
 */
export function trimActivity(activityId: string, startIndex: number, endIndex: number): Activity | null {
  const activities = readAll()
  const activity = activities.find((a) => a.id === activityId)
  if (!activity) return null

  const clampedStart = Math.max(0, Math.min(startIndex, activity.points.length - 1))
  const clampedEnd = Math.max(clampedStart, Math.min(endIndex, activity.points.length - 1))
  const trimmedPoints = activity.points.slice(clampedStart, clampedEnd + 1)
  if (trimmedPoints.length < 2) return activity

  activity.points = trimmedPoints
  activity.startedAt = trimmedPoints[0].timestamp
  if (activity.endedAt != null) {
    activity.endedAt = trimmedPoints[trimmedPoints.length - 1].timestamp
  }
  writeAll(activities)
  return activity
}
